...
class MEISTER extends Page
{
    ...
    {
        $recordset = $this->_database->query("SELECT * FROM fragen ORDER BY rand() LIMIT 1;");
        if (!$recordset) {
...
EOT;
    ...
    protected function processReceivedData()
    {
        if (isset($_POST['name'])) {
            ...
            $ok = $this->_database->query("INSERT INTO spiele (name, antworten) VALUES ('$name', 0);");
            if (!$ok) {
                throw new Exception("Fehler bei Insert");
            }
            $_SESSION['spielID'] = $this->_database->insert_id;
        }
        ...
        if (isset($_SESSION['spielID'])) {
            ...
            if ($this->richtig) {
                $ok = $this->_database->query("UPDATE spiele SET antworten = antworten + 1 WHERE id = '{$_SESSION['spielID']}';");
                if (!$ok) {
